...
def _get_temperature(*keys: str) -> float | None:
    """Read the first present temperature env var and parse as float."""
    for key in keys:
        raw = os.getenv(key)
        ...
        try:
            ...
            raise ValueError(
                f"Invalid temperature in {key}='{raw}'. Expected a float."
            ) from exc
    return None
...
def _load_openai(model_name: str):
    ...
    temperature = _get_temperature("OPENAI_TEMPERATURE", "LLM_TEMPERATURE")
...
# Default judge models per provider — prefer stronger reasoning models
...
def load_judge_model():
    """
    Load a judge LLM using JUDGE_PROVIDER / JUDGE_MODEL env vars.

    Intentionally separate from load_model() so the judge can use a
    different provider/model than the agent, avoiding self-preference bias.

    Environment Variables
    ---------------------
    JUDGE_PROVIDER : Provider for the judge. Defaults to 'openai'.
    JUDGE_MODEL    : Model name for the judge. Falls back to a default per provider.

    Returns
    -------
    BaseChatModel with temperature=0 for deterministic, consistent scoring.
    """
    provider = _get_env("JUDGE_PROVIDER", default="openai").lower()
    if provider not in _PROVIDER_LOADERS:
        raise ValueError(
            f"Unsupported judge provider '{provider}'. "
            f"Choose one of: {', '.join(_PROVIDER_LOADERS.keys())}"
        )

    model_name = _get_env("JUDGE_MODEL", default=_DEFAULT_JUDGE_MODELS.get(provider, "gpt-5-mini"))

    # Force temperature=0 for the judge — deterministic scoring is essential
    # for reproducible experiment comparisons.
    original_temps = {}
    for key in ("OPENAI_TEMPERATURE", "LLM_TEMPERATURE"):
        original_temps[key] = os.environ.pop(key, None)
    os.environ["LLM_TEMPERATURE"] = "0"

    try:
        judge = _PROVIDER_LOADERS[provider](model_name)
    finally:
        # Restore original temperature env vars so the agent is unaffected
        os.environ.pop("LLM_TEMPERATURE", None)
        for key, val in original_temps.items():
            if val is not None:
                os.environ[key] = val

    # print(f"[model_loader] Judge  — Provider: {provider} | Model: {model_name} | Temperature: 0")
    return judge
